...
/**
 * Comprobante de reserva imprimible. Incluye banner de email simulado,
 * resumen completo (código, huésped, habitación, extras, método de pago)
 * y botón de "Imprimir / Guardar Voucher PDF" vía window.print().
 */
export default function VoucherConfirmation({ booking }: { booking: Booking }) {
  if (!booking) return null;

  const nights = nightsBetween(booking.checkIn, booking.checkOut);
  const roomName = booking.room?.name;
  const roomNumber = booking.room?.number;
  const extras: Extra[] = booking.extrasSeleccionados || [];
    ...
    <div className="voucher-page">
      ...
      <article className="card voucher-card" id="voucher">
        ...
        <footer className="voucher-foot muted small">
          <p>
            {site.name} · Reserva directa. Presentá este comprobante en el check-in junto con
