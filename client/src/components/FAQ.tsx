...
type FaqWithAnswerId = Faq & { answerId: string };

function FaqItem({ item }: { item: FaqWithAnswerId }) {
...
const faqs: Faq[] = [
  ...
  },
  {
    ...
    a: 'Aceptamos efectivo, transferencia local, tarjetas de crédito o débito y pago online con Mercado Pago. Pagás directamente en el alojamiento: nunca cobramos nada por adelantado.',
  },
  {
    id: 'crib',
...
/**
 * Acordeón de preguntas frecuentes con animación de altura nativa
 * (grid-template-rows + overflow hidden). La sección se revela al entrar
 * en el viewport.
 */
export default function FAQ() {
  ...
  return (
    <section ref={ref} className={`landing-section reveal${visible ? ' reveal-visible' : ''}`}>
